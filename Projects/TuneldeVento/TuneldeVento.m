...
xlabel('X');
ylabel('Y');
title('Campo de Pressão Dobrada');
axis equal;
...
% Inicializar a força de sustentação
F_lift = 0;

% Definindo deltaA como a área de um elemento de malha
...
% Percorre a malha para calcular a pressão na carroceria e armazenar os valores
for j = 1:Ny
    for i = 1:Nx
        if external_mask(j,i) && Y(j,i)>Yc
            % Calcula a normal à superfície em cada ponto (j, i)
            dx = X(j,i) - Xc;
            dy = Y(j,i) - Yc;
            n = [dx; dy] / norm([dx; dy]); % Normal unitária

            % Calcula a contribuição da pressão no ponto (j, i)
            if i~=Nx
                P = 2*p(j,i); % Pressão no ponto
            else
                P = p(j,i); % Pressão no ponto
            end
            F_lift = F_lift + P * n(2) * deltaA; % Considerando apenas a componente vertical
